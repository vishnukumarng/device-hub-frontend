...
import { User, Mail, LogOut, CheckCircle2, History, ListChecks, CalendarClock, Clock } from "lucide-react";
...
export default function Profile() {
  ...
  const { currentUser, logout, checkouts, waitlist } = useApp();
    ...
    <div className="space-y-6 select-none">
      ...
      <div className="grid grid-cols-3 gap-3">
        <Card className="border-border bg-card text-center p-3 flex flex-col items-center justify-center">
          ...
          <span className="text-lg font-bold text-white leading-tight">{activeCount}</span>
          <span className="text-[10px] text-muted-foreground font-medium uppercase tracking-wider mt-0.5">Active</span>
        </Card>
        <Card className="border-border bg-card text-center p-3 flex flex-col items-center justify-center">
          <CalendarClock className="w-4 h-4 text-warning mb-1.5" />
          <span className="text-lg font-bold text-white leading-tight">{reserveCount}</span>
          <span className="text-[10px] text-muted-foreground font-medium uppercase tracking-wider mt-0.5">Reserved</span>
        </Card>
        <Card className="border-border bg-card text-center p-3 flex flex-col items-center justify-center">
          <Clock className="w-4 h-4 text-emerald-400 mb-1.5" />
          <span className="text-lg font-bold text-white leading-tight">{waitlistCount}</span>
          <span className="text-[10px] text-muted-foreground font-medium uppercase tracking-wider mt-0.5">Waitlisted</span>
        </Card>
      </div>
            ...
            <Card key={hist.id} className="border-border bg-card p-3.5">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <h4 className="text-xs font-semibold text-white truncate">{hist.deviceName}</h4>
                  <p className="text-[10px] text-muted-foreground capitalize mt-0.5 leading-normal">{hist.category?.toLowerCase()}</p>
                </div>
                <div className="text-right shrink-0 flex flex-col items-end">
                  ...
                  <span className="text-[9px] text-muted-foreground/50 mt-0.5">{hist.returnedAt}</span>
                </div>
              </div>
            </Card>
          ))}
